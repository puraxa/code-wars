function transformString(str) {
  let transData = str.split("\n");
  for (let i = 0; i < transData.length; i++) {
    transData[i] = transData[i].split(':');
    transData[i][1] = transData[i][1].split(',');
    for (let k = 0; k < transData[i][1].length; k++) {
      transData[i][1][k] = transData[i][1][k].split(' ');
    }
  }

  let arrObj = transData.reduce((acc , element) => {
    acc[element[0]] = [];
    for (let i = 0; i < element[1].length; i++) {
      acc[element[0]].push(parseFloat(element[1][i][1]));
    }
    return acc;
  },{});
  return arrObj;
...
function mean(town, strng) {
  let transformed = transformString(strng);
  if(!transformed[town]){
    return -1;
  }
  let sum = 0;
  for (let  i = 0; i < transformed[town].length; i++){
    sum += transformed[town][i];
  }
  return sum/transformed[town].length;
}
function variance(town, strng) {
  let mid = mean(town, strng);
  let transformed = transformString(strng);
  if(!transformed[town]){
    return -1;
  }
  let res = 0;
  for(let i = 0; i < transformed[town].length; i++){
    res += Math.pow((transformed[town][i] - mid),2);
  }
  return res/transformed[town].length;
}
